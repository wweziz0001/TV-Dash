...
- `player/floating-player.ts`
  - pure helpers for floating-player layout defaults, viewport clamping, and stacking order
...
- player diagnostics should distinguish between native browser PiP and TV-Dash floating fallback so surrounding pages can explain which playback mode is active
...
- native PiP should be attempted first for the active player when the browser allows it
- when native PiP is unavailable, already occupied, or rejected by the browser session, TV-Dash may fall back to an in-page floating player without tearing down the live video element
- multiple TV-Dash floating players may coexist because they are managed inside the page rather than by the browser PiP window manager
